...
//UPLOAD MARK SHEET BY TEACHERS
exports.uploadMarkSheet = catchAsyncErrors(async (req, res, next) => {
  const session = req.body.session;
  const course = req.user.course;
  const department = req.user.department;
  const semester = req.body.semester;

  const getMarkSheet = Marks.findOne({
    session: session,
    course: course,
  ...
  if (getMarkSheet) {
    // update mark sheet here
  } else {
    // upload Mark Sheet here
